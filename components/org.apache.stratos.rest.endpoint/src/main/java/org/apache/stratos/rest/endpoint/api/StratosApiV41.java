...
public class StratosApiV41 extends AbstractApi {
    ...
    public Response getIaasProviders()
            throws RestAPIException {
        IaasProvidersInfoBean iaasProvidersInfoBean = StratosApiV41Utils.getIaasProviders();
        if (iaasProvidersInfoBean == null) {
            return Response.status(Response.Status.NOT_FOUND).entity(new ResponseMessageBean(
                    ResponseMessageBean.ERROR, "No IaaS Providers found")).build();
        }
        return Response.ok(iaasProvidersInfoBean).build();
    }
}
